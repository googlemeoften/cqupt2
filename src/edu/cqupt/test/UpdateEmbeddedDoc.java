 ...
 * 
 * <p>
 * This code was successfully tested with the following file from the POI test
 * collection: http://svn.apache.org/repos/asf/poi/trunk/test-data/document/
...
public class UpdateEmbeddedDoc {
	 ...
	 *            and path to a WordprocessingML Word document that contains an
	 *            embedded binary Excel workbook.
	 * @throws java.io.FileNotFoundException
	 *             Thrown if the file cannot be found on the underlying file
	 *             system.
	 * @throws java.io.IOException
	 *             Thrown if a problem occurs in the underlying file system.
	 */
	public UpdateEmbeddedDoc(String filename) throws FileNotFoundException,
			IOException {
	 ...
	 *             Rather than use the specific classes (HSSF/XSSF) to handle
	 *             the embedded workbook this method uses those defeined in the
	 *             SS stream. As a result, it might be the case that a
	 *             SpreadsheetML file is opened for processing, throwing this
	 *             exception if that file is invalid.
	 * @throws java.io.IOException
	 *             Thrown if a problem occurs in the underlying file system.
	 */
		...
		if (embeddedDocs != null && !embeddedDocs.isEmpty()) {
			...
			while (pIter.hasNext()) {
				...
				if (pPart.getPartName().getExtension().equals(BINARY_EXTENSION)
						|| pPart.getPartName().getExtension()
								.equals(OPENXML_EXTENSION)) {
	 ...
	 * updated. This method simply recovers the first cell from the first row of
	 * the first workbook and tests the value it contains.
	 * <p/>
	 * Note that execution will not continue up to the assertion as the embedded
	 * workbook is now corrupted and causes an IllegalArgumentException with the
	 * following message
	 * <p/>
	...
	public static void main(String[] args) {
		try {
			UpdateEmbeddedDoc ued = new UpdateEmbeddedDoc("F://simple.docx");
